# Documentation index

This is the entry point for understanding and operating the repository.

## Start here

- [Project purpose](project-purpose.md)
- [Repository architecture](../ARCHITECTURE.md)
- [Repository map](repository-map.md)
- [Current repository baseline](../project/status/repository-baseline.md)
- [Agent operating contract](../AGENTS.md)
- [Contribution guide](../CONTRIBUTING.md)

## Architecture

- [Information flow](architecture/information-flow.md)
- [Knowledge flow](architecture/knowledge-flow.md)
- [Agent operating model](architecture/agent-operating-model.md)
- [Publication pipeline](architecture/publication-pipeline.md)

## Concepts

- [Information objects and lifecycle boundaries](concepts/information-objects.md)
- [Source, evidence, and knowledge](concepts/source-evidence-knowledge.md)
- [Knowledge object model](concepts/knowledge-object-model.md)

## Governance

- [Documentation policy](governance/documentation-policy.md)
- [Change management](governance/change-management.md)
- [Information handling](../project/information-handling.md)
- [Source management policy](../project/source-management-policy.md)
- [Knowledge management policy](../project/knowledge-management-policy.md)
- [Knowledge extraction guidelines](../project/knowledge-extraction-guidelines.md)
- [Knowledge review workflow](../project/knowledge-review-workflow.md)
- [Architecture Decision Records](../project/decisions/)
- [Architectural-decision review status](../project/status/architectural-decisions.md)

## Operations and reference

- [Local development](operations/local-development.md)
- [Using the Codex prompt library](operations/using-codex-prompt-library.md)
- [Creating knowledge records](operations/creating-knowledge-records.md)
- [Reviewing knowledge records](operations/reviewing-knowledge-records.md)
- [Traversing knowledge relationships](operations/traversing-knowledge-relationships.md)
- [Identifier rules](reference/identifiers.md)
- [Knowledge identifiers](reference/knowledge-identifiers.md)
- [Status values](reference/statuses.md)
- [Knowledge statuses](reference/knowledge-statuses.md)
- [Project status](../project/status/)
- [Master implementation roadmap](../project/roadmap/implementation-roadmap.md)
- [Master roadmap status](../project/status/master-roadmap-status.md)
- [Source catalogue](../sources/catalogue.yaml)
- [Controlled configuration](../config/)
- [Schemas](../schemas/)

Specialized directory README files provide local scope rules. Where they
conflict materially, follow the precedence in `AGENTS.md` and report the
conflict.
