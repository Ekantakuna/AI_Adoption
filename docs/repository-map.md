...
| `project/roadmap.yaml`, `project/roadmap/` | Machine-readable and human-readable stage sequencing, gates, lifecycle, and prompt boundary | Dates, unsupported estimates, execution claims, or automatic approval | Architecture, policies, status, schemas, and validation evidence | Human stage planning and one-at-a-time prompt generation | `scripts/validate_roadmap.py` and tests | Partial; Stage 9.5 draft in review |
